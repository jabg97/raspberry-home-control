...
/**
 * @Route("/gpio/{id}/{signal}", name="gpioUpdate", defaults={"id" = null,"signal" = null}, requirements={"signal" = "[0-9]{1,1}"} )
 * @Method({"GET"})
 * @Security("has_role('ROLE_ADMIN') or has_role('ROLE_USER')")
 */
    public function gpioUpdateAction(Request $request, $id = null, $signal = null)
    {
        $em = $this->get('doctrine')->getManager();

        $device = $em->getRepository('AppBundle:Dispositivos')->find($id);
 ...
 */
    ...
    private function backupAction(Request $request, $pin1 = null, $pin2 = null, $signal = null)
    {
        if ($signal == 0 || $signal == 1) {

            $comando = "gpio mode " . $pin1 . " out";
            exec($comando);
            $comando = "gpio write " . $pin1 . " " . $signal;
            exec($comando);
